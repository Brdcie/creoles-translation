...
def remove_silent_letters(phrase):
    ...
    for word in words:
        ...
        word = re.sub(r'([bcdfghjklmnpqrstvwxz])c$', '\g<1>', word)  # Supprimer 'c' final après consonne    
        word = re.sub(r'(?<![dcjgqz])e$', '', word)  # Supprimer 'e' final sauf après 'd' ou 'c' j g z q
...
def apply_creole_rules(phrase):
    ...
    transformation_rules = [
        ...
        (r'(?<![ao])u', 'i'),   # Remplacer 'u' par 'i' sauf après 'a' et 'o'
        # Règle 11  suite pour 's' entre voyelles : 's' -> 'z'
        #(r'(?<!ç)(?<=[aeéiouyAEIOUY])s(?=[aeéiouyAEIOUY])(?!yon)', 'z'),
...
def transform_french_to_creole(phrase):
    ...
    for word in words:
        ...
        if word_lower in SPECIAL_WORDS:
            ...
            # Étape 1 : Supprimer les lettres muettes
            word = remove_silent_letters(word)
